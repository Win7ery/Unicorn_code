...
def load_model(fh):
    """Load a model from a file handle @fh."""
    model_name = None
    num_cluster = None
    medoids = []
    mean_thresholds = []
    ...
    for line in fh:
        ...
        if line.startswith("model: "):
            ...
            num_cluster = int(line.split(": ")[1])
        elif line.startswith("medoid: "):
            medoid = [float(elem) for elem in line.split(": ")[1].split()]
            medoids.append(medoid)
        elif line.startswith("mean: "):
            mean_thresholds = [float(elem) for elem in line.split(": ")[1].split()]
        elif line.startswith("max: "):
            max_thresholds = [float(elem) for elem in line.split(": ")[1].split()]
        elif line.startswith("std: "):
            stds = [float(elem) for elem in line.split(": ")[1].split()]
        elif line.startswith("evolution: "):
            evolution = [int(evol) for evol in line.split(": ")[1].split()]

    model = Model(model_name.split("/")[-1])
    model.model_name = model_name
    model.num_cluster = num_cluster
    model.medoids = medoids
    model.mean_thresholds = mean_thresholds
    model.max_thresholds = max_thresholds
    model.stds = stds
...
def load_sketches(fh):
    ...
    # code add by yan
    sketches_list = [list(sketch) for sketch in sketches]
...
if __name__ == "__main__":
    ...
    fh = open(model_save_path, 'r')
    models = dict()
    model = load_model(fh)
    models[model_save_path.split('/')[-1]] = model
